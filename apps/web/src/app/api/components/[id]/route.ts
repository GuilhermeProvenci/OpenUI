  ...
  try {
    ...
    if (!component.published && component.authorId !== session?.user?.id) {
      return Response.json(
        { error: "This component was removed by its author" },
        { status: 410 },
      );
    }
    ...
    console.error("[GET /api/components/[id]] stack:", err.stack);
    return Response.json(
...
export async function PUT(
  ...
  try {
    ...
    const { component, error: authorError } = await requireAuthor(
      id,
      session!.user!.id,
    );
